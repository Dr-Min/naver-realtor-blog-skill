...
고칠 파일은 `profile.yaml` 하나뿐입니다. 스킬 문서는 직접 수정하지 마세요.
업데이트할 때 충돌합니다.

```bash
node ~/.codex/skills/naver-realtor-blog/scripts/init-profile.mjs --scope home
```

생성 위치는 `~/.codex/naver-realtor-blog/profile.yaml`입니다. 사무소를 여러 개
관리한다면 작업 폴더에 `--scope project`로 하나 더 만들어 항목별로 덮어쓸 수
있습니다.

| 바꾸고 싶은 것 | 고칠 항목 |
| --- | --- |
| 사무소명·중개사명·연락처·영업시간 | `office.*` |
| 주력 지역과 매물 종류 | `specialties.*` |
| 글의 톤 | `style.source`, `style.preset`, `style.card` |
| 인사말·상담 문구·이모지 허용 | `style.overrides.*` |
| 절대 쓰면 안 되는 표현 | `prohibited.claims` |
| 비용 모드 | `run.execution_mode` |
| 조사 범위(DataLab·자동완성·블로그검색) | `run.research.*` |
| QA 통과 후 자동 임시저장 여부 | `run.save_after_qa` |

사무소 정보를 한 번 적어두면 다음 글부터는 매물 사실만 물어봅니다.

### 글의 톤은 세 겹으로 정해집니다

| 층 | 내용 | 바뀌나 |
| --- | --- | --- |
| 가독성 하한선 | `naver_mobile_v1` | 바뀌지 않음. QA가 강제합니다 |
| 톤 프리셋 | 담백형 · 친근형 · 정보형 | 기존 블로그 글이 없을 때 |
| 스타일 카드 | 내 블로그에서 학습한 문체 | 있으면 프리셋을 대신합니다 |

`style.overrides`에 적은 값은 어느 경우에도 가장 먼저 적용됩니다.

### 기존 블로그 글이 있다면 문체를 학습시킬 수 있습니다

`profile.yaml`의 `learning.own_blog_url`에 본인 블로그 주소를 적고 이렇게
요청하세요.

> 내 블로그 문체 학습해줘

최근 글 5개를 읽고 문체 카드를 한 장 만듭니다. 원문을 저장하지 않고 요약만
남기므로, 이후 글을 쓸 때 추가 토큰이 들지 않습니다.

카드에는 **따라할 것**과 **따라하지 않을 것**이 이유와 함께 나뉘어 기록됩니다.
어미와 인사말, 자주 쓰는 표현, 소제목 방식, 상담 문구는 가져오고, 지나치게 긴
문단이나 `역세권 최저가` 같은 근거 없는 최상급 표현은 이유를 붙여 제외합니다.
목록을 보고 마음에 들지 않는 항목은 빼달라고 하면 됩니다.

학습 대상은 **본인 소유 블로그만**입니다. 다른 사람의 글은 문체 학습에
쓰지 않습니다. 경쟁 글 분석이 필요하면 3C 블로그 검색 단계가 담당합니다.

갱신은 수동입니다. 다시 요청할 때까지 카드는 그대로 유지됩니다.

기존 글이 없거나 예전 문체를 쓰고 싶지 않다면 `style.source`를 `preset`으로
두면 됩니다. 기본값이라 아무것도 하지 않아도 됩니다.

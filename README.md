...
| 역할 | 기본 모델 | 입력 | 출력 |
| --- | --- | --- | --- |
| DataLab 수집 | `gpt-5.6-luna high` | 후보군, 필터, 기간 | 3A 원본·화면·결과 |
| 자동완성 수집 | `gpt-5.6-luna high` | 기준 검색어, 세션 상태 | 3B 원본·화면·결과 |
| 블로그 검색 수집 | `gpt-5.6-luna high` | 검색어, 정렬, 표본 수 | 3C 원본·화면·결과 |
| 키워드 결정 | `gpt-5.6-sol high` | 잠긴 3A·3B·3C | 선택·기각·반론 |
| 콘텐츠 기획·작성 | `gpt-5.6-sol high` | 입력, 키워드, 사진, 독자 가설 | 모바일 원고·가독성 검수값·페이로드 |
| 예상 독자 QA | `gpt-5.6-sol high` | 입력, 사진 관찰, 독자 가설 | 독자 QA |
| 증거 QA | `gpt-5.6-sol high` | 원본, 화면, URL, 해시 | 증거 QA |
| 콘텐츠 QA | `gpt-5.6-sol high` | 입력, 원고, 페이로드 | 콘텐츠 QA |
| 브라우저 담당 | `gpt-5.6-terra high` | 잠긴 페이로드 | 임시저장 결과 |
| 보고서 담당 | `gpt-5.6-terra high` | 잠긴 전체 산출물 | HTML 보고서 |

모델을 사용할 수 없으면 가장 가까운 역할의 모델로 대체하고 실제 모델과 변경 이유를 기록합니다.
